...
namespace SıgnalRWebUI.Controllers
{
    public class FeatureController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7001/api/Feature");//GetAsync HttpClient da var olan metot.Nereye Get isteğinde bulunacaksam orada var olan adresi alıyorum. 
        ...
        public async Task<IActionResult> CreateFeature(CreateFeatureDto createFeatureDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7001/api/Feature", stringContent);
        ...
        public async Task<IActionResult> DeleteFeature(int id)
        {
            ...
            var responseMessage = await client.DeleteAsync($"https://localhost:7001/api/Feature/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7001/api/Feature/{id}");//İlk olarak güncellemem gereken id yi bulmalıyım.

            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync(); //json dan gelen içeriği string formatında oku
                var values = JsonConvert.DeserializeObject<UpdateFeatureDto>(jsonData);//güncelleyeceğim veriyi dto aracılığı ile dönmeliyim.Json veriyi metne dönüştürüyorum.
                return View(values);
            }
            return View();
        ...
        public async Task<IActionResult> UpdateFeature(UpdateFeatureDto updateFeatureDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7001/api/Feature", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
